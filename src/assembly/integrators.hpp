...
namespace mpfem {
    ...
    public:
        explicit DiffusionIntegrator(const VariableNode* c)
            : coef_(c)
        {
            if (!coef_)
                MPFEM_THROW(ArgumentException, "DiffusionIntegrator requires non-null matrix node");
        }
    ...
    public:
        explicit MassIntegrator(const VariableNode* c)
            : coef_(c)
        {
            if (!coef_)
                MPFEM_THROW(ArgumentException, "MassIntegrator requires non-null scalar node");
        }
    ...
    public:
        explicit DomainLFIntegrator(const VariableNode* c)
            : coef_(c)
        {
            if (!coef_)
                MPFEM_THROW(ArgumentException, "DomainLFIntegrator requires non-null scalar node");
        }
     ...
     */
    class BoundaryLFIntegrator : public FacetLinearIntegratorBase {
    public:
        explicit BoundaryLFIntegrator(const VariableNode* c)
            : coef_(c)
        {
            if (!coef_)
                MPFEM_THROW(ArgumentException, "BoundaryLFIntegrator requires non-null scalar node");
        }

        void assembleFacetVector(const ReferenceElement& ref,
            ElementTransform& trans,
            Vector& elvec) const override;
     ...
     */
    class ConvectionMassIntegrator : public FacetBilinearIntegratorBase {
    public:
        explicit ConvectionMassIntegrator(const VariableNode* c)
            : coef_(c)
        {
            if (!coef_)
                MPFEM_THROW(ArgumentException, "ConvectionMassIntegrator requires non-null scalar node");
        }

        void assembleFacetMatrix(const ReferenceElement& ref,
            ElementTransform& trans,
            Matrix& elmat) const override;
     ...
     */
    class ConvectionLFIntegrator : public FacetLinearIntegratorBase {
    public:
        ConvectionLFIntegrator(const VariableNode* h, const VariableNode* Tinf)
            : coef_(h), Tinf_(Tinf)
        {
            if (!coef_)
                MPFEM_THROW(ArgumentException, "ConvectionLFIntegrator requires non-null h node");
            if (!Tinf_)
                MPFEM_THROW(ArgumentException, "ConvectionLFIntegrator requires non-null Tinf node");
        }
    ...
    public:
        ...
        {
            if (!E_)
                MPFEM_THROW(ArgumentException, "ElasticityIntegrator requires non-null E node");
            if (!nu_)
                MPFEM_THROW(ArgumentException, "ElasticityIntegrator requires non-null nu node");
            if (vdim_ <= 0)
                MPFEM_THROW(ArgumentException, "ElasticityIntegrator requires vdim > 0");
        }
    ...
    public:
        ...
        {
            if (!stress_)
                MPFEM_THROW(ArgumentException, "StrainLoadIntegrator requires non-null stress node");
            if (vdim_ <= 0)
                MPFEM_THROW(ArgumentException, "StrainLoadIntegrator requires vdim > 0");
        }
