...
inline void ensureStorage(Matrix& m, int rows, int cols)
{
...
void GeometryMapping::evalShape(Geometry geom, int order, const Vector3& xi, Matrix& shape)
{
    if (order < 1 || order > 2) {
    ...
    case Geometry::Segment:
        if (order == 1) {
            ensureStorage(shape, 2, 1);
            shape(0, 0) = 0.5 * (1.0 - x);
            shape(1, 0) = 0.5 * (1.0 + x);
            return;
        }
        ensureStorage(shape, 3, 1);
...
void GeometryMapping::evalDerivatives(Geometry geom, int order, const Vector3& xi, Matrix& derivatives)
{
    if (order < 1 || order > 2) {
        MPFEM_THROW(NotImplementedException, "GeometryMapping supports only order 1 and 2");
    }
    ...
    case Geometry::Segment:
        if (order == 1) {
            ensureStorage(derivatives, 2, 3);
            derivatives(0, 0) = -0.5;
            derivatives(1, 0) = 0.5;
            return;
        }
        ensureStorage(derivatives, 3, 3);
    ...
    case Geometry::Tetrahedron:
        ...
        ensureStorage(derivatives, 10, 3);
    ...
    case Geometry::Cube:
        ...
        ensureStorage(derivatives, 27, 3);
        {
            const Real px0 = -0.5 * x * (1.0 - x);
            const Real px1 = 1.0 - x * x;
            const Real px2 = 0.5 * x * (1.0 + x);
