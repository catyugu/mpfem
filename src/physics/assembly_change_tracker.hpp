...
inline constexpr std::uint64_t AssemblyTagSeed = 1469598103934665603ull;
...
requires(std::integral<std::remove_cvref_t<IntegerT>> &&
         !std::same_as<std::remove_cvref_t<IntegerT>, bool> &&
         !std::same_as<std::remove_cvref_t<IntegerT>, std::uint64_t>)
inline std::uint64_t stateTagOf(IntegerT value) {
    return static_cast<std::uint64_t>(value);
}

inline std::uint64_t stateTagOf(const std::set<int>& ids) {
    std::uint64_t tag = AssemblyTagSeed;
    for (int id : ids) {
