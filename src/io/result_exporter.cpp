...
namespace mpfem
{
                                          ...
                                          const std::string &filename)
    {
        ...
        for (int i = 0; i < result.numTimeSteps(); ++i)
        {
            ...
            {
                tFields[i] = &fields.current(FieldId::Temperature);
            }
            if (fields.hasField(FieldId::Displacement))
            {
                uFields[i] = &fields.current(FieldId::Displacement);
            }
        }
                                              ...
                                              const std::string &filename, Real time)
    {
        ...
        const GridFunction *V = fields.hasField(FieldId::ElectricPotential)
                                    ? &fields.current(FieldId::ElectricPotential)
                                    : nullptr;
        const GridFunction *T = fields.hasField(FieldId::Temperature)
                                    ? &fields.current(FieldId::Temperature)
                                    : nullptr;
        const GridFunction *u = fields.hasField(FieldId::Displacement)
                                    ? &fields.current(FieldId::Displacement)
                                    : nullptr;
