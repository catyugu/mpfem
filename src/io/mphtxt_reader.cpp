...
namespace mpfem {
    ...
    Mesh MphtxtReader::readFile(const std::string& filename, Real scaleFactor)
    {
        ...
        for (const auto& block : data.blocks) {
            ...
            int meshDim = data.sdim;

            // Add all entities by dimension — strata handles separation of
            // volume (dim=meshDim), boundary (dim=meshDim-1), and lower-dim
            // entities (dim < meshDim-1, e.g. vertices and edges in 3D).
            // Boundary entity attributes are 0-indexed in COMSOL; shift to 1-indexed.
            for (size_t i = 0; i < block.elements.size(); ++i) {
                Index attr = 0;
                if (i < block.geomIndices.size()) {
                    attr = block.geomIndices[i];
                }
                if (entityDim == meshDim) {
                    mesh.addEntity(entityDim, geom, block.elements[i], attr, block.order);
                    numVolumeElems++;
                } else if (entityDim == meshDim - 1) {
                    // Boundary elements: shift from 0-indexed COMSOL to 1-indexed
                    mesh.addEntity(entityDim, geom, block.elements[i], attr + 1, block.order);
                    numBdrElems++;
                } else {
                    // Lower-dim entities (vertices, edges): no attribute shift needed
                    mesh.addEntity(entityDim, geom, block.elements[i], attr, block.order);
                }
            }
        }
