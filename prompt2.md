...
* 拆分BilinearForm与BoundaryBilinearForm，LinearForm与BoundaryLinearForm的基类，避免产生冗余的虚函数与接口。
...
或许是你使用的积分规则不够好？我找不到其他来由……

## 架构修复

* 耦合场例如JouleHeating必须相对独立，以添加耦合项的形式加入总求解流程，而非直接把焦耳热等效应注入单场求解器中。
* 单场求解器只应该包含自己的边界条件设置接口、源项设置接口等。
