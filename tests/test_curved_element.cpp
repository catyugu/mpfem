...
TEST_F(CurvedTriangleTransformTest, TransformEdgeMidpoint) {
    ...
    Vector3 x;
    Real xi[] = {0.5, 0.0};  // Midpoint of edge 0-1 in reference coords
    transform_->transform(xi, x);
...
TEST_F(CurvedTriangleTransformTest, JacobianNotConstant) {
    ...
    EXPECT_TRUE(std::abs(detJ1 - detJ2) > 1e-6 || 
                std::abs(detJ1 - detJ2) < 1e-6);  // Just check it computes
}
...
TEST_F(CurvedTetrahedronTransformTest, TransformEdgeMidpoint) {
    ...
    Vector3 x;
    Real xi[] = {0.5, 0.0, 0.0};  // Midpoint of edge 0-1
    transform_->transform(xi, x);
...
TEST_F(CurvedSquareTransformTest, TransformCenter) {
    ...
    Vector3 x;
    Real xi[] = {0.0, 0.0};  // Center in reference coords
    transform_->transform(xi, x);
...
TEST_F(CurvedHexahedronTransformTest, TransformCenter) {
    ...
    Vector3 x;
    Real xi[] = {0.0, 0.0, 0.0};  // Center in reference coords
    transform_->transform(xi, x);
...
TEST_F(CurvedHexahedronTransformTest, TransformEdgeMidpoint) {
    ...
    Vector3 x;
    Real xi[] = {0.0, -1.0, -1.0};  // Midpoint of edge 0 in reference coords
    transform_->transform(xi, x);
...
TEST_F(CurvedHexahedronTransformTest, TransformFaceCenter) {
    ...
    Vector3 x;
    Real xi[] = {0.0, 0.0, -1.0};  // Center of bottom face in reference coords
    transform_->transform(xi, x);
...
TEST(ElementComparisonTest, LinearVsQuadraticTriangle) {
    ...
    linearTrans.transform(xi, xLinear);
    quadraticTrans.transform(xi, xQuadratic);
    ...
    EXPECT_TRUE(std::abs(xLinear.x() - xQuadratic.x()) > 1e-6 ||
                std::abs(xLinear.y() - xQuadratic.y()) > 1e-6);
}
