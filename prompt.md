...
特别说明：
* 1. 三棱柱 (Wedge/Prism) 单元：处理相对简单。它的参考元可以看作是“底面三角形 $\times$ 高度一维线段”。它的形函数直接是 2D 三角形形函数和 1D 线段形函数的张量积（Tensor Product）。参考坐标通常设为 $(\xi, \eta, \zeta)$，其中 $\xi, \eta$ 遵循三角形面积坐标规则，$\zeta \in [-1, 1]$。雅可比矩阵的计算与六面体无异。
* 2. 金字塔 (Pyramid) 单元：这是个棘手的家伙。金字塔底部是四边形，顶部是一个顶点。难点： 如果你试图用标准的多项式去构造参考金字塔的形函数，你会发现在顶点（Apex）处，雅可比矩阵会奇异（分母为零或导数不唯一）。解决方案 （退化法）： 把一个标准的八节点六面体单元的顶面四个节点“捏合”成一个节点（坐标相同）。
