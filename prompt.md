...
* 测试要求：基于案例的测试
* 能复用的代码（主要是从mfem）尽可能不重写，只需要封装与适配。能通过加入简单依赖的实现尽量避免手写。
...
* 边界条件：
  * 电势：boundary 1电势为Vtot，boundary 8和15电势为0（接地）。其余绝缘。
  * 固体传热：boundary 1-7, 9-14, 16-42有对流换热边界，换热系数为htc，外界温度为293.15K，其余为热绝缘。
  * 刚体力学：boundary 8, 15, 43 为固定（零位移），其余均自由。
* 考虑焦耳热和热膨胀。
